...
class SolicitudPlanta extends Component
{
    ...
    public function confirmar($id)
    {
        ...
        if ($detalle->tipo_analisis == "Fisicoquimico") {
            ...
            // Verificar si el detalle ya existe en la tabla de actividad


            if ($detalle->tipo_muestra_id == 1) {
                $existe = AguaFisico::where('detalle_solicitud_planta_id', $detalle->id)->exists();
                if ($existe) {
                     $this->dispatch('error_mensaje', mensaje: 'Ya se registro la muestra');
                ...
                $existe = ActividadAgua::where('detalle_solicitud_planta_id', $detalle->id)->exists();
                if ($existe) {
                    $this->dispatch('error_mensaje', mensaje: 'Ya se registro la muestra');
        ...
        if ($detalle->tipo_analisis == "Microbiologico") {
            ...
            // Verificar si el detalle ya existe en la tabla de actividad
            $existe = MicrobiologiaExterno::where('detalle_solicitud_planta_id', $detalle->id)->exists();
            if ($existe) {
