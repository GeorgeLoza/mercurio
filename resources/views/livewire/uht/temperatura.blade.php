<div>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg h-[calc(40vh)] bg-white dark:bg-gray-900 ">
        <table class="w-full text-xs text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="sticky top-0 z-10 text-xs text-center text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-1 py-1">tempUHT</th>
            ...
            <tbody class="">
                @foreach ($resultados as $index => $resultado)
                    <tr
                        ...
                        <td class="px-1 py-2">
                            @if ($editing === $index)
                                <input type="text" wire:model.defer="resultados.{{ $index }}.tempUHT"
                                    class="w-20 px-1 py-1 border rounded">
                            @else
                                <span>{{ $resultado->tempUHT ?? '-' }}</span>
                            @endif
                        </td>
