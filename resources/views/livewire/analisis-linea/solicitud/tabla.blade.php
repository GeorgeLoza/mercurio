        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 ">
            ...
            <tbody class="">
                ...
                <tr
                    ...
                    <th >
                        @foreach ($solicitud->estadoPlanta->estadoDetalle as $detalles)
                        <p class="whitespace-nowrap" data-popover-target="popover-{{$detalles->id}}">{{Str::limit($detalles->orp->producto->nombre,20)}}</p>
                    ...
                    <td class="flex items-center px-2 py-2 gap-2">
                        @if(now()->diffInMinutes($solicitud->created_at)<90) <svg
                            onclick="Livewire.dispatch('openModal', { component: 'analisis-linea.solicitud.editar', arguments: { id: {{ $solicitud->id}} } })"
                            ...
                            @endif
                            @if(in_array(auth()->user()->rol, ['Admi','Sup','HTST','UHT']))
